/* main.js - Application Bootloader & UI Controller */

import { loadState, saveState, initDB } from './utils/storage.js';
...
function triggerBackgroundAudit(patientId) {
...
function renderPatientsList() {
    ...
    appState.patients.forEach(patient => {
        ...
        tr.querySelector(`#btn-view-profile-${patient.id}`).onclick = () => {
            if (appState.currentUser.role === 'receptionist') {
                triggerBackgroundAudit(patient.id);
...
function cancelAppointmentFlow() {
    const id = document.getElementById('appointment-id-field').value;
    if (!id) return;

    try {
        cancelAppointment(id, appState, appState.systemTime);
        saveState(appState);
        loadCalendar();
        closeAppointmentModal();
    } catch (err) {
...
document.getElementById('add-evolution-note-form').addEventListener('submit', (e) => {
    e.preventDefault();
    const patientId = document.getElementById('ehr-patient-select').value;
    if (!patientId) return;
    ...
    addEvolutionNote(patientId, noteText, appState.currentUser.name, systemTimeStr, appState);
    saveState(appState);
...
function createToothElement(tId, patient) {
    ...
    // SVG template mapping click actions
    const svgStr = `
        <svg class="tooth-svg" viewBox="0 0 50 50" width="40" height="40">
            <polygon points="0,0 50,0 35,15 15,15" class="surface buccal" data-surface="buccal"></polygon>
            <polygon points="50,0 50,50 35,35 35,15" class="surface distal" data-surface="distal"></polygon>
            <polygon points="0,50 50,50 35,35 15,35" class="surface lingual" data-surface="lingual"></polygon>
            <polygon points="0,0 0,50 15,35 15,15" class="surface mesial" data-surface="mesial"></polygon>
            <polygon points="15,15 35,15 35,35 15,35" class="surface occlusal" data-surface="occlusal"></polygon>
        </svg>
        <svg class="absent-cross" viewBox="0 0 50 50" width="40" height="40">
            <line x1="5" y1="5" x2="45" y2="45" stroke="#ef4444" stroke-width="3"></line>
            <line x1="45" y1="5" x2="5" y2="45" stroke="#ef4444" stroke-width="3"></line>
        </svg>
    `;
...
function saveBaselineStateFlow() {
    const patientId = document.getElementById('ehr-patient-select').value;
    if (!patientId) return;
...
function applyTreatmentToTooth() {
    const patientId = document.getElementById('ehr-patient-select').value;
    if (!patientId) return;
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    if (!id) {
        ...
        if (idx > -1) {
            ...
            if (password) {
                appState.users[idx].passwordHash = await hashPassword(password);
            }
        }
    }
...
function renderAuditLogs() {
    ...
    appState.auditLogs.forEach(log => {
        ...
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><code>${log.id.split('-')[1]}</code></td>
...
async function bootstrap() {
    ...
    // Load local database state into active application memory
    const loaded = loadState();
    appState.users = loaded.users;
    appState.patients = loaded.patients;
    appState.shifts = loaded.shifts;
    appState.appointments = loaded.appointments;
    appState.auditLogs = loaded.auditLogs;
