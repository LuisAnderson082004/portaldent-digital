...
// Import newly extracted sub-modules for clinical/administrative sections
import { 
    renderTarifarioList, 
    openTarifarioModal, 
    closeTarifarioModal, 
...
// -------------------------------------------------------------
// BUDGET & DISCOUNTS VIEW LOGIC
// -------------------------------------------------------------
function setupBudgetPatientSearch() {
...
function initializeBudgetView() {
    ...
    if (patientId) {
        ...
        if (patient) {
            ...
            const clearBtn = document.getElementById('budget-patient-search-clear');
            if (clearBtn) clearBtn.classList.remove('hidden');
            loadBudgetForPatient(patientId);
        }
    } else {
        clearBudgetPanel();
    }
...
async function loadBudgetForPatient(patientId) {
    const patient = appState.patients.find(p => p.id === patientId);
    if (!patient) return;
        ...
        tr.innerHTML = `
            ...
            <td>
                <input type="number" min="0" max="100" class="form-control budget-discount-input" 
                       ...
                       oninput="recalculateBudgetRow(this)">
            </td>
...
function recalculateRowValues(tr, discountPercent) {
...
function recalculateBudgetRow(input) {
...
function recalculateBudgetTotals(hasDeposit) {
    let subtotal = 0;
    let totalDiscount = 0;
    ...
    document.getElementById('budget-subtotal').innerText = `S/ ${subtotal.toFixed(2)}`;
    document.getElementById('budget-total-discount').innerText = `S/ ${totalDiscount.toFixed(2)}`;
    document.getElementById('budget-deposit').innerText = `S/ ${depositAmount.toFixed(2)}`;
    document.getElementById('budget-final-total').innerText = `S/ ${finalTotal.toFixed(2)}`;
...
async function saveBudgetDiscounts() {
    const select = document.getElementById('budget-patient-select');
    const patientId = select ? select.value : '';
    if (!patientId) {
...
// Bind save button explicitly too
document.getElementById('btn-save-budget-discounts').addEventListener('click', saveBudgetDiscounts);

// Bind save button explicitly too
document.getElementById('btn-save-budget-discounts').addEventListener('click', saveBudgetDiscounts);

// -------------------------------------------------------------
// ORTHODONTICS VIEW LOGIC (Ficha y Control Mensual)
// -------------------------------------------------------------
// BIND GLOBAL WINDOW EVENT HANDLERS FOR BACKWARD COMPATIBILITY
// -------------------------------------------------------------
window.switchView = switchView;
