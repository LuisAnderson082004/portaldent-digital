...
import { exportPatientPDFDirect, printBudgetSheet, printOrthoControlSheet } from './utils/pdf-generator.js';
import { getResendApiKey, setResendApiKey, sendAppointmentReminderEmail } from './utils/resendClient.js';
...
import { 
    ...
    openTarifarioModal, 
    closeTarifarioModal, 
    deleteTarifarioCatalogItem, 
    setupTarifario 
} from './modules/tarifario.js';
import { 
    initializeTreatmentPlanView, 
    loadPatientTreatmentPlans, 
    toggleTreatmentPlanStatus, 
    deleteTreatmentPlanItem, 
    setupTreatmentPlan 
} from './modules/treatmentPlan.js';
import { 
    setupBudgetPatientSearch, 
    clearBudgetPanel, 
    initializeBudgetView, 
    loadBudgetForPatient, 
...
import { 
    switchOrthoTab, 
    initializeOrthoHistoryView, 
    saveOrthoHistory, 
    initializeOrthoControlsView, 
    renderOrthoControlsTable, 
    openOrthoControlModal, 
    closeOrthoControlModal, 
    saveOrthoControlFlow, 
    deleteOrthoControlFlow, 
    printOrthoControl, 
    setupOrthodontics 
} from './modules/orthodontics.js';
...
let currentOdontogramMode = 'baseline'; 
...
function switchView(viewId) {
    ...
    switch (viewId) {
        ...
        case 'treatment-plan':
            ...
            initializeTreatmentPlanView(appState);
            break;
        case 'tarifario':
            ...
            renderTarifarioList(appState);
            break;
        case 'budget':
            viewTitle.innerText = "Presupuestos y Descuentos";
            viewSubtitle.innerText = "Detalle de costos de tratamientos planificados, descuentos y deducciones por abono.";
            initializeBudgetView(appState);
            break;
        case 'ortho-history':
            viewTitle.innerText = "Historia Clínica de Ortodoncia";
            viewSubtitle.innerText = "Ficha clínica de diagnóstico, análisis de modelos y hábitos de ortodoncia.";
            initializeOrthoHistoryView(appState);
            break;
        case 'ortho-controls':
            viewTitle.innerText = "Control y Seguimiento de Ortodoncia";
            viewSubtitle.innerText = "Evolución clínica mensual, reparaciones y derivaciones de ortodoncia.";
            initializeOrthoControlsView(appState);
            break;
    }
}
...
function renderShiftsList() {
    ...
    if (!appState.shifts || appState.shifts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted py-4">No hay turnos registrados para los odontólogos. Haga clic en "+ Registrar Nuevo Turno" para asignar horarios.</td></tr>';
        return;
    }

    appState.shifts.forEach(s => {
        const dentistName = s.dentistName || s.nombre_odontologo || 'Odontólogo';
        const dayVal = s.day || s.dayOfWeek || s.dia_semana || '';
        const startVal = s.start || s.startTime || s.hora_inicio || '';
        const endVal = s.end || s.endTime || s.hora_fin || '';

        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><strong>${dentistName}</strong></td>
            <td>${getDaySpanish(dayVal)}</td>
            <td>${formatTime12Hour(startVal)}</td>
            <td>${formatTime12Hour(endVal)}</td>
            <td>
                <button class="btn btn-danger-outline btn-sm" id="btn-delete-shift-${s.id}"><i class="fa-solid fa-trash"></i> Eliminar</button>
            </td>
        `;
        
        const delBtn = tr.querySelector(`#btn-delete-shift-${s.id}`);
        if (delBtn) delBtn.onclick = () => deleteShift(s.id);
        tbody.appendChild(tr);
    });
}
...
function openShiftModal() {
    ...
    toggleModal('shift-modal', true);
}

function closeShiftModal() {
    toggleModal('shift-modal', false);
}
...
function renderPatientsList() {
    ...
    appState.patients.forEach(patient => {
        ...
        tr.querySelector(`#btn-view-profile-${patient.id}`).onclick = async () => {
            try {
                const clinical = await getPatientClinicalData(patient.id);
...
async function editPatient(patientId) {
    ...
    try {
        ...
        console.error("Error loading patient clinical data for edit:", err.message);
    }
...
async function togglePatientStatus(patientId) {
    ...
    try {
        ...
        console.error("Error loading patient clinical data for toggle status:", err.message);
    }
...
function closePatientModal() {
    toggleModal('patient-modal', false);
}
...
function handleSlotClick(date, time, dentistId, appointment) {
    ...
    toggleModal('appointment-modal', true);
}
...
function closeAppointmentModal() {
    toggleModal('appointment-modal', false);
}
...
function openReminderTemplateModal(apptId) {
    ...
    const templateText = `Estimado(a) ${appt.patientName}, le recordamos su cita dental programada para la fecha ${appt.date} a las ${formattedTime} con el especialista ${appt.dentistName} en la Clínica Portal Dentistas. Agradecemos su puntualidad.`;

    document.getElementById('reminder-template-text').innerText = templateText;

    // Populate recipient email input (default to luisanderson082004@gmail.com if no email registered)
    const patient = appState.patients.find(p => p.id === appt.patientId);
    const emailInput = document.getElementById('reminder-patient-email');
    if (emailInput) {
        emailInput.value = (patient && (patient.email || patient.correo)) || 'luisanderson082004@gmail.com';
    }

    toggleModal('reminder-modal', true);
}

function closeReminderModal() {
    toggleModal('reminder-modal', false);
}

function saveResendKeyFromUI(key) {
    setResendApiKey(key);
    const keyStatus = document.getElementById('resend-key-status');
    if (keyStatus) {
        keyStatus.innerText = key ? "Configurada" : "Sin configurar";
        keyStatus.className = key ? "badge badge-success" : "badge badge-warning";
    }
}

async function sendResendReminderEmail() {
    if (!activeReminderApptId) return;

    const appt = appState.appointments.find(a => a.id === activeReminderApptId);
    if (!appt) return;

    const emailInput = document.getElementById('reminder-patient-email');
    const patientEmail = emailInput ? emailInput.value.trim() : '';

    if (!patientEmail) {
        alert("Ingrese un correo electrónico de paciente válido.");
        return;
    }

    const btn = document.getElementById('btn-send-resend-email');
    if (btn) {
        btn.disabled = true;
    ...
    try {
        ...
        alert(`✅ Recordatorio enviado exitosamente por correo electrónico a ${patientEmail} mediante Resend API.`);
        ...
        if (index > -1) {
            ...
            try {
                ...
                console.error("Error actualizando estado de recordatorio en BD:", dbErr.message);
            }
        }
        closeReminderModal();
    } catch (err) {
        alert("Error al enviar el correo con Resend API: " + err.message);
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fa-solid fa-paper-plane"></i> Enviar Email (Resend)`;
        }
    }
}
...
async function loadEHRForPatient() {
    ...
    try {
        ...
        console.error("Error loading patient clinical data for EHR view:", err.message);
    }
...
function createToothElement(tId, patient) {
    ...
    activeFindings.forEach(f => {
        const color = f.estado ? '#0043c7' : '#ef4444'; 
    ...
    surfaces.forEach(poly => {
        ...
        if (activeSurf) {
            ...
            const fillColor = activeSurf.estado ? '#0043c7' : '#ef4444';
...
function openFindingModal(tId, surface) {
    ...
    const stateSelect = document.getElementById('finding-state-select');

    typeSelect.onchange = () => {
        const val = typeSelect.value;
        const pathologyTypes = ['C', 'DES', 'FRA', 'RR'];
        if (pathologyTypes.includes(val)) {
            stateSelect.value = "false"; // Mal estado / Requerido / Color Rojo (#ef4444)
        } else if (val) {
            stateSelect.value = "true"; // Buen estado / Existente / Color Azul (#0043c7)
        }
    };
...
document.getElementById('odontogram-finding-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        patient.odontogram = patient.odontogram || { baseline: {}, evolution: {} };
...
window.saveResendKeyFromUI = saveResendKeyFromUI;
window.sendResendReminderEmail = sendResendReminderEmail;
...
window.exportPatientPDF = () => {
    let patientId = document.getElementById('budget-patient-select')?.value;
...
window.openTarifarioModal = (itemId) => openTarifarioModal(itemId, appState);
window.closeTarifarioModal = closeTarifarioModal;
window.deleteTarifarioCatalogItem = (id) => deleteTarifarioCatalogItem(id, appState);


window.toggleTreatmentPlanStatus = (planId, currentStatus) => toggleTreatmentPlanStatus(planId, currentStatus, appState);
window.deleteTreatmentPlanItem = (planId) => deleteTreatmentPlanItem(planId, appState);


window.saveBudgetDiscounts = () => saveBudgetDiscounts(appState);
window.recalculateBudgetRow = (input) => recalculateBudgetRow(input, appState);


window.switchOrthoTab = switchOrthoTab;
window.initializeOrthoHistoryView = () => initializeOrthoHistoryView(appState);
window.initializeOrthoControlsView = () => initializeOrthoControlsView(appState);
window.openOrthoControlModal = () => openOrthoControlModal(appState);
window.closeOrthoControlModal = closeOrthoControlModal;
window.deleteOrthoControlFlow = (controlId) => deleteOrthoControlFlow(controlId, appState);
window.printOrthoControl = (controlId) => printOrthoControl(controlId, appState);
...
async function bootstrap() {
    ...
    setupPatientSearch();
    setupBudgetPatientSearch(appState);
    setupSidebarToggle();

    
    setupTarifario(appState);
    setupTreatmentPlan(appState);
    setupBudget(appState);
    setupOrthodontics(appState);
}
