...
function switchOrthoTab(tabId, buttonEl) {
...
async function initializeOrthoHistoryView() {
    const patientId = document.getElementById('ehr-patient-select').value;
    ...
    try {
        ...
        if (record) {
            ...
            document.querySelectorAll('#ortho-models-table-body tr').forEach((tr, idx) => {
                ...
                if (row.type === 'linea_media') {
                    tr.querySelector('.models-sup-type').value = analysis.linea_media_sup_type || 'Normal';
                    tr.querySelector('.models-sup-val').value = analysis.linea_media_sup_val || '';
                    tr.querySelector('.models-inf-type').value = analysis.linea_media_inf_type || 'Normal';
                    tr.querySelector('.models-inf-val').value = analysis.linea_media_inf_val || '';
                } else {
                    tr.querySelector('.models-sup').value = analysis[`${row.key}_sup`] || '';
                    tr.querySelector('.models-inf').value = analysis[`${row.key}_inf`] || '';
                }
            });
...
async function saveOrthoHistory(e) {
    e.preventDefault();
    const patientId = document.getElementById('ehr-patient-select').value;
    if (!patientId) return;
    ...
    try {
        const saved = await saveOrthoRecord(record);
...
// Monthly controls variables
let activeControlsList = [];

async function initializeOrthoControlsView() {
    const patientId = document.getElementById('ehr-patient-select').value;
...
async function renderOrthoControlsTable(patientId) {
...
function openOrthoControlModal() {
    const modal = document.getElementById('ortho-control-modal');
    document.getElementById('ortho-control-form').reset();
...
async function deleteOrthoControlFlow(controlId) {
...
function printOrthoControl(controlId) {
    const patientId = document.getElementById('ehr-patient-select').value;
    const patient = appState.patients.find(p => p.id === patientId);
...
// Bind Orthodontic actions to window
window.switchOrthoTab = switchOrthoTab;
window.initializeOrthoHistoryView = initializeOrthoHistoryView;
window.initializeOrthoControlsView = initializeOrthoControlsView;
window.openOrthoControlModal = openOrthoControlModal;
window.closeOrthoControlModal = closeOrthoControlModal;
window.deleteOrthoControlFlow = deleteOrthoControlFlow;
window.printOrthoControl = printOrthoControl;

// Listeners
document.getElementById('ortho-history-form').addEventListener('submit', saveOrthoHistory);
document.getElementById('ortho-control-form').addEventListener('submit', saveOrthoControlFlow);
