...
export async function renderTarifarioList(appState) {
    ...
    if (!appState || !appState.treatmentsCatalog || appState.treatmentsCatalog.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">No hay procedimientos registrados en el catálogo.</td></tr>';
        return;
    }
